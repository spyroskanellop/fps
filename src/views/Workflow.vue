<template>
  <v-container>
      ...
      <v-col class="d-flex align-center justify-end kanban-col">
        <div class="d-block">
          ...
          <div class="d-flex align-end justify-end ga-2 mt-5">
            <v-btn class="btn-label" @click="filterByPriority('high_priority')">High Priority</v-btn>
            <v-btn class="btn-label" @click="filterByPriority('due_today')">Due Today</v-btn>
            <v-btn class="btn-label" @click="filterByPriority('due_tomorrow')">Due Tomorrow</v-btn>
            <v-btn class="btn-label" @click="filterByPriority('newest')">Newest</v-btn>
            <v-btn class="btn-label" @click="filterByPriority('oldest')">Oldest Waiting</v-btn>
          </div>
        </div>
      </v-col>
    </v-row>
    ...
    <v-row no-gutters class="ga-8 work-container mt-8">
      <v-col v-for="column in columns" :key="column.name" class="work-column">
        <v-row>
          <v-col>
    ...
    fetchOrders() {
      getOrders()
        .then((response) => {
          ...
          console.log("Orders fetched successfully:", this.columns);
        })
        .catch((error) => {
